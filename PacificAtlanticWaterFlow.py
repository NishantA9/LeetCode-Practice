...
class Solution:
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        pac, atl = set(), set()  # Cells reachable by Pacific and Atlantic oceans
        directions = [[1, 0], [-1, 0], [0, 1], [0, -1]]        
        def dfs(r, c, visit, prevHeight): # DFS to mark all cells that can flow into the given ocean
            if ((r, c) in visit or r < 0 or c < 0 or r == ROWS or c == COLS or heights[r][c] < prevHeight):
                return  # Boundary conditions + height restriction
            visit.add((r, c))
            for dr, dc in directions:
                dfs(r + dr, c + dc, visit, heights[r][c])
            # dfs(r + 1, c, visit, heights[r][c]) # Explore in 4 directions
            # dfs(r - 1, c, visit, heights[r][c])
            # dfs(r, c + 1, visit, heights[r][c])
            # dfs(r, c - 1, visit, heights[r][c])
        for c in range(COLS): # Start DFS from all Pacific-bordering cells (top and left edges)
        ...
        res = [] #  Find cells reachable by both oceans
        for r in range(ROWS):
            for c in range(COLS):
