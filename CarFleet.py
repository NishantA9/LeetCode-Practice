...
class Solution:
    def carFleet(self, target: int, position: List[int], speed: List[int]) -> int:
        ...
        pair = [[p, s] for p, s in zip(position, speed)] # Pair positions and speeds, then sort by position in descending order
        for p, s in sorted(pair)[::-1]:  # Reverse sorted order by position
            stack.append((target - p) / s) # Calculate the time for the car to reach the target
